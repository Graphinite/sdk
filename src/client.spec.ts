...
import { GraphiniteClient } from './client';


let token: string;
let client: GraphiniteClient;
...
describe('Test client class', () => {
  test('Test default url', () => {
    const cli = new GraphiniteClient(token);
    expect((cli as any).url).toBe('https://europe-west1-graphinite-core.cloudfunctions.net/api_v0');
  });

  test('Test provided url', () => {
    const cli = new GraphiniteClient(token, { apiUrl: 'http://localhost:3000' });
  ...
  test('Test provided malformed url', () => {
    const run = () => new GraphiniteClient(token, { apiUrl: 'failure' });
    expect(run).toThrow();
  });
});
...
describe('Test client functions', () => {
  ...
  describe('Test sendRequest()', () => {
    ...
    beforeEach(() => {
      ...
      client = new GraphiniteClient(token, { apiUrl: 'http://localhost:3000' });
      fetchMock.mockReset();
    });
